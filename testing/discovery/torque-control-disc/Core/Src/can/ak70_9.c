/*
 * ak70_9.c — CubeMars AK70-9 KV60 Motor CAN driver (VESC protocol).
 * Uses can_send_ext() from the CAN API transport layer.
 */
...
void pack_cmd(uint8_t controller_id, float p_des, float v_des, float kp, float kd, float t_ff) {
    ...
    buffer[0] = (uint8_t)(kp_int >> 4);
    buffer[1] = (uint8_t)(((kp_int & 0xF) << 4) | (kd_int >> 8));
    buffer[2] = (uint8_t)(kd_int & 0xFF);
    buffer[3] = (uint8_t)(p_int >> 8);
    buffer[4] = (uint8_t)(p_int & 0xFF);
    buffer[5] = (uint8_t)(v_int >> 4);
    buffer[6] = (uint8_t)(((v_int & 0xF) << 4) | (t_int >> 8));
    buffer[7] = (uint8_t)(t_int & 0xFF);
