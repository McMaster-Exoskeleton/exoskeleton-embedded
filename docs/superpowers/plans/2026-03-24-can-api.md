...
| Modify | `apis/motor/Src/can/ak70_9.c` | Migrate from can_bus.h to can_common.h |
| Modify | `apis/motor/Inc/can/ak70_9.h` | Update include from can_frame.h to can_common.h |
...
int can_common_init(CAN_HandleTypeDef *hcan, uint8_t my_node_id) {
    ...
    f.FilterMaskIdHigh     = (0x0780 << 5);  /* 0xF000 — match type bits [10:7] only */
    ...
    f.FilterIdLow          = 0x0000;
    f.FilterMaskIdHigh     = (0x078F << 5);  /* 0xF1E0 — match type + dest bits */
    f.FilterMaskIdLow      = 0x0000;
    f.FilterFIFOAssignment = CAN_FILTER_FIFO0;
    f.FilterActivation     = ENABLE;
    f.SlaveStartFilterBank = 14;
...
```bash
git add apis/can/python/can_common.py
...
### Task 9: Python Package Init — __init__.py

**Files:**
- Create: `apis/can/python/__init__.py`

This task must come AFTER Tasks 5-8 so all submodules exist when the package is importable.
...
```bash
git add apis/can/python/__init__.py
git commit -m "feat(can-api): add Python package __init__.py"
```

---

### Task 10: Migrate ak70_9.c to use can_common

**Files:**
- Modify: `apis/motor/Src/can/ak70_9.c`

The existing motor API depends on `can_bus.h` / `can_bus.c` which is superseded by `can_common.h` / `can_common.c`. Since the STM32 HAL only allows one `HAL_CAN_RxFifo0MsgPendingCallback` per compilation unit, both files cannot coexist in a build. This task updates ak70_9 to use the new CAN API transport.

Note: `ak70_9.h` does NOT include `can_frame.h` or `can_bus.h` — it only uses `<stdint.h>` and defines its own structs. No changes needed to the header.

- [ ] **Step 1: Update ak70_9.c includes**
...
Note: The build system (Makefile/CMake/CubeIDE) will need `apis/can/Inc/` added to the include path so `can_common.h` resolves.

- [ ] **Step 2: Update comm_can_transmit_eid in ak70_9.c**

Replace the call to `can_bus_send_ext` with `can_send_ext`:

```c
// In comm_can_transmit_eid():
// Old: can_bus_send_ext(id, data, len);
// New: can_send_ext(id, data, len);
```

- [ ] **Step 3: Note on can_bus_recv migration**

`ak70_9.c` does NOT call `can_bus_recv` — motor feedback is received in the application layer (e.g., `main.c`). Any calls to `can_bus_recv()` in `main.c` or other application code should be updated to `can_recv()` when integrating, but that is outside this API task's scope.

- [ ] **Step 4: Commit**

```bash
git add apis/motor/Src/can/ak70_9.c
git commit -m "refactor(motor-api): migrate ak70_9 from can_bus to can_common"
```

---

### Task 11: Documentation — CAN_API.md
