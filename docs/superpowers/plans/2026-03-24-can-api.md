...
- Modify: `apis/motor/Inc/can/ak70_9.h`
...
- [ ] **Step 3: Update motor_recv_feedback in ak70_9.c**

Replace the call to `can_bus_recv` with `can_recv`:

```c
// Old: can_bus_recv(&frame);
// New: can_recv(&frame);
```

- [ ] **Step 4: Update ak70_9.h include**
...
The `CanFrame` struct is identical in both headers, so no other changes are needed.

- [ ] **Step 5: Commit**

```bash
git add apis/motor/Src/can/ak70_9.c apis/motor/Inc/can/ak70_9.h
